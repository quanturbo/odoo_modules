...
from .purchase_request_linking import open_linked_purchase_orders

REQUEST_PRIORITY_SELECTION = [
    ("0", "Low"),
    ("1", "Normal"),
    ("2", "High"),
    ("3", "Urgent"),
...
class StockRequest(models.Model):
    ...
    priority = fields.Selection(
        selection=REQUEST_PRIORITY_SELECTION,
        default="1",
        index=True,
        tracking=True,
    )
    ...
    allocation_ids = fields.One2many(
        ...
        string="Inventory Request Allocation",
    )
    ...
    procurement_group_id = fields.Char()
    company_id = fields.Many2one()
    route_id = fields.Many2one()

    _name_uniq = models.Constraint(
        "UNIQUE (name, company_id)", "Inventory Request name must be unique"
    )
        ...
        for rec in self:
            if rec.state == "draft" and rec.product_qty <= 0:
                raise ValidationError(
                    _("Inventory Request product quantity has to be strictly positive.")
                )
            elif rec.state != "draft" and rec.product_qty < 0:
                raise ValidationError(
                    _("Inventory Request product quantity cannot be negative.")
                )

    @api.depends("purchase_line_ids", "purchase_line_ids.order_id")
    ...
    def _check_purchase_company_constraints(self):
        if any(
    ...
    def action_cancel(self):
        ...
        if not self.env.context.get("skip_cancel_po_from_inventory_request"):
            requests = self.sudo()
            requests.purchase_ids.filtered(
                lambda order: order.state not in ("purchase", "done", "cancel")
                and not (order.stock_request_ids - requests)
            ).button_cancel()
        return True
    ...
    def _prepare_procurement_values(self, group_id=False):
        ...
        coming from a inventory request. This method could be override
    ...
    def _action_launch_procurement_rule(self):
        ...
        inventory request. stock rule will launch '_run_move',
        '_run_buy' or '_run_manufacture'
        depending on the inventory request product rule.
        """
        precision = self.env["decimal.precision"].precision_get(
            "Product Unit of Measure"
        )
    ...
    def action_view_purchase(self):
        return open_linked_purchase_orders(self, self.mapped("purchase_ids"))

    def _prepare_values_from_order(self, order):
        return {
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if upd_vals.get("product_id") and not upd_vals.get("product_uom_id"):
            ...
            if "order_id" in upd_vals:
                ...
                for field_name, field_value in self._prepare_values_from_order(order_id).items():
                    upd_vals.setdefault(field_name, field_value)
            if "expected_date" not in upd_vals:
