...
class PurchaseOrderLine(models.Model):
    ...
    stock_request_ids = fields.Many2many(
        comodel_name="stock.request", string="Inventory Requests", copy=False
    )
    ...
    def unlink(self):
        ...
        """
        requests_to_cancel = self.env["stock.request"]
        for purchase_line in self:
            requests_to_cancel |= purchase_line.stock_request_ids
        res = super().unlink()
        if requests_to_cancel:
            requests_to_cancel.action_cancel()
        return res

    def _prepare_stock_moves(self, picking):
        res = super()._prepare_stock_moves(picking)
        for move_values in res:
            move_values["allocation_ids"] = [
                Command.create(
                    {
    ...
    def _prepare_purchase_order_line_from_procurement(
        ...
        return attach_request_to_purchase_values(vals, values)

    @api.constrains("stock_request_ids", "company_id")
        ...
        if any(
            ...
            raise ValidationError(
                _(
                    ...
                    "to an inventory request that belongs to "
                    "another company."
                )
            )
