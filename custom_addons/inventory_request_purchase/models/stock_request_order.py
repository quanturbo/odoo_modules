...
from .stock_request import REQUEST_PRIORITY_SELECTION


class StockRequestOrder(models.Model):
    _name = "stock.request.order"
    _description = "Inventory Request Order"
    ...
    procurement_group_id = fields.Char(
        "Procurement Group",
        help="Moves created through this inventory request will be put in this "
    ...
    stock_request_count = fields.Integer(
        string="Inventory Requests", compute="_compute_stock_request_count", readonly=True
    )
    ...
    route_id = fields.Many2one(
        ...
        readonly=True,
        store=True,
        help="The route related to a inventory request order",
    )
    ...
    @api.depends(
        "stock_request_ids",
        "stock_request_ids.purchase_ids",
        "stock_request_ids.purchase_line_ids",
    )
    def _compute_purchase_ids(self):
        for order in self:
            order.purchase_ids = order.stock_request_ids.mapped("purchase_ids")
            order.purchase_line_ids = order.stock_request_ids.mapped("purchase_line_ids")
    ...
    def action_view_inventory_requests(self):
        action = self.env["ir.actions.act_window"]._for_xml_id(
