...
class StockRequest(models.AbstractModel):
    ...
    _description = "Inventory Request Template"
    ...
    allowed_uom_categ_id = fields.Many2one(
        ...
        compute="_compute_allowed_uom_categ_id",
    )
    product_uom_id = fields.Many2one(
        comodel_name="uom.uom",
        string="Product Unit of Measure",
        domain="[('id', 'child_of', allowed_uom_categ_id)]",
        required=True,
    ...
    @api.depends("product_id.uom_id", "product_id.uom_id.relative_uom_id")
    def _compute_allowed_uom_categ_id(self):
        for request in self:
            uom = request.product_id.uom_id
            while uom.relative_uom_id:
                uom = uom.relative_uom_id
            request.allowed_uom_categ_id = uom
    ...
    def _check_product_uom(self):
        """Check if the UoM belongs to the product standard UoM tree."""
        if any(
            request.product_uom_id
            and request.allowed_uom_categ_id
            and not request.product_uom_id.parent_path.startswith(
                request.allowed_uom_categ_id.parent_path
            )
            for request in self
        ):
            raise ValidationError(
                _(
                    ...
                    "same unit tree than the default unit "
    ...
    def onchange_warehouse_id(self):
        ...
        if self._name == "stock.request" and self.order_id:
            # When the inventory request is created from an order the wh and
